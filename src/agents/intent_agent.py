...
- teacher_exams -- exams created by teachers (name only). Supports: count, list. Can filter
  ...
  are the application's own persisted workflow states), or by term (a dynamic lookup filter --
  term labels are real per-school data, e.g. "Term 1" or "Final", not a fixed list). No
  grouping, no sorting, no date querying.
