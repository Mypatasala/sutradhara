...
def test_prompt_teacher_profiles_bullet_documents_department_filter():
    bullet = _teacher_profiles_bullet()
    assert "department" in bullet.lower()


def test_prompt_teacher_profiles_bullet_still_documents_employment_type():
    """Regression: confirms the pre-existing EMPLOYMENT_TYPE capability
    (and its exact vocabulary) is unchanged by adding the DEPARTMENT
    filter."""
    bullet = _teacher_profiles_bullet()
    assert "FULL_TIME/PART_TIME/CONTRACT/VISITING" in bullet


def test_prompt_teacher_profiles_bullet_includes_department_worked_example():
    bullet = _teacher_profiles_bullet()
